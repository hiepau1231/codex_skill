...
**Errors**: Poll `failed` → retry once; `timeout`/`stalled` → report partial results from `review.raw_markdown`, suggest lower effort; `error` → report to user. Start/resume `error` with `CODEX_NOT_FOUND` → tell user to install codex. Agent failures → degrade gracefully, partial coverage. If Codex + all agents fail → fallback to inline Claude review (single-reviewer, all categories). Always run cleanup.
